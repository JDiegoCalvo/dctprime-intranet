		...
		for ( var i = 0; i < r.diario.length; i++ )
		{
			...
			HTML += `
				<li>
					...
					if ( r.diario[i].forma_de_pago !== null )
					{
						HTML += `
						<table class="table table-sm table-bordered">
							<thead>
								<tr>
									...
									<th>Cta. de destino</th>
									<th>Banco de destino</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td>${ r.diario[i].forma_de_pago }</td>
									<td>${ r.diario[i].cta_origen }</td>
									<td>${ r.diario[i].banco_origen }</td>
									<td>${ r.diario[i].cta_destino }</td>
									<td>${ r.diario[i].banco_destino }</td>
								</tr>
							</tbody>
						</table>`;
					}
